  ...
  Biller.fromJson(Map j)
      : id = j['id'],
        ...
        fields = (j['fields'] as List)
            .map((e) => FieldDef.fromJson(e as Map))
            .toList();
...
String? validateField(FieldDef f, String? v) {
  ...
  if (!RegExp(f.regex).hasMatch(t))
    return 'Enter a valid ${f.label.toLowerCase()} (${f.hint})';
  return null;
}
...
String? validateAmount(int paise, int due, bool allowPartial) {
  ...
  if (!allowPartial && paise != due)
    return 'This biller does not allow partial payments';
  return null;
}
...
class SavedBiller {
  ...
  SavedBiller copyWith({bool? autopay, bool? paused, int? maxPaise}) =>
      SavedBiller(
          id: id,
          billerId: billerId,
  ...
  factory SavedBiller.fromJson(Map<String, dynamic> j) => SavedBiller(
      id: j['id'],
      billerId: j['billerId'],
      billerName: j['billerName'],
      category: j['category'],
      nickname: j['nickname'],
      fields: Map<String, String>.from(j['fields'] as Map),
      autopay: j['autopay'] == true,
      paused: j['paused'] == true,
      maxPaise: j['maxPaise'] ?? 0);
}
...
class Bill {
  ...
  bool get noDue =>
      amountPaise <= 0; // "No bill due" is a normal state, not an error
  bool get settled => totalPaise > 0 && amountPaise <= 0;
  int get daysLeft =>
      dueDate == null ? 999 : dueDate!.difference(today()).inDays;
...
class Payment {
  ...
  factory Payment.fromJson(Map<String, dynamic> j) => Payment(
      ...
      account: j['account'],
      status: PayStatus.values.byName(j['status']),
      billerRef: j['billerRef'],
      bankRef: j['bankRef'],
...
class UpcomingItem {
  ...
  bool get needsApproval =>
      ...
      saved.autopay &&
      !saved.paused &&
      bill!.amountPaise > saved.maxPaise;
}
